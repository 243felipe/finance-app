...
@Component({
  selector: 'app-receita-renda-extra-relatorio',
  standalone: true,
  imports: [CommonModule, FormsModule, TableModule, ButtonModule, DatePickerModule],
  templateUrl: './receita-renda-extra-relatorio.component.html',
  styleUrls: ['./receita-renda-extra-relatorio.component.scss']
})
export class ReceitaRendaExtraRelatorioComponent implements OnInit {
  ...
  pesquisar(): void {
    ...
    this.service.listByDateRange({
      ...
      next: (data) => {
        // Filtrar receitas que não são salário (sem fonte de renda ou fonte diferente)
  ...
  gerarPdf(): void {
    ...
    const titulo = `Relatório de Receita - Renda Extra - ${periodo}`;
