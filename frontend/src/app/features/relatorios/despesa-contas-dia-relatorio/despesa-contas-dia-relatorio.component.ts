...
@Component({
  selector: 'app-despesa-contas-dia-relatorio',
  standalone: true,
  imports: [CommonModule, FormsModule, TableModule, ButtonModule, DatePickerModule],
  templateUrl: './despesa-contas-dia-relatorio.component.html',
  styleUrls: ['./despesa-contas-dia-relatorio.component.scss']
})
export class DespesaContasDiaRelatorioComponent implements OnInit {
  ...
  pesquisar(): void {
    ...
    this.service.listByDateRange({
      ...
      next: (data) => {
        // Filtrar despesas que não são conta fixa
        this.dados = data.filter(l => (l.idContaFixa === null || l.idContaFixa === undefined) && (l.idFonteRenda === null || l.idFonteRenda === undefined));
        this.loading = false;
      },
      error: (err) => {
  ...
  gerarPdf(): void {
    ...
    const titulo = `Relatório de Despesa - Contas do Dia - ${periodo}`;
