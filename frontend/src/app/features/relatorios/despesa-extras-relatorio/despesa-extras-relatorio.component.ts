...
@Component({
  selector: 'app-despesa-extras-relatorio',
  standalone: true,
  imports: [CommonModule, FormsModule, TableModule, ButtonModule, DatePickerModule],
  templateUrl: './despesa-extras-relatorio.component.html',
  styleUrls: ['./despesa-extras-relatorio.component.scss']
})
export class DespesaExtrasRelatorioComponent implements OnInit {
  ...
  pesquisar(): void {
    ...
    this.service.listByDateRange({
      ...
      next: (data) => {
        // Filtrar despesas extras (sem conta fixa, mas pode ter outras características)
  ...
  gerarPdf(): void {
    ...
    const titulo = `Relatório de Despesa - Extras - ${periodo}`;
