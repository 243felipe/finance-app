...
@Component({
  selector: 'app-receita-renda-extra',
  standalone: true,
  imports: [CommonModule, FormsModule, ReactiveFormsModule, TableModule, ButtonModule, DialogModule, InputTextModule],
  templateUrl: './receita-renda-extra.component.html',
  styleUrls: ['./receita-renda-extra.component.scss']
})
export class ReceitaRendaExtraComponent implements OnInit {
  ...
  constructor(
    ...
    this.form = this.fb.group({
      ...
      idFonteRenda: [null, [Validators.required]], // renda extra exige fonte
      idContaFixa: [null],
    ...
    forkJoin([
      ...
      next: ([lanc, cats, fontes, contas, formas]) => {
        this.itens = (lanc || []).filter(
          (l: LancamentoFinanceiro) => l.tipo === 'E' && l.idFonteRenda !== null && l.idFonteRenda !== undefined
        ); // renda extra: receita + fonte definida
  ...
  salvar(): void {
    ...
    this.saving = true;
    const payload = { ...this.form.value, tipo: 'E' } as Omit<
